...
# ==============================================================================
    ...
    with aba_auto:
                    ...
                    if not ia_disponivel:
                        ...
                        with st.spinner("A IA está analisando os diagramas P&ID..."):
                            for arq in arquivos_diagrama:
                                try:
                                    if arq.name.lower().endswith('.pdf'): doc_input = {"mime_type": "application/pdf", "data": arq.getvalue()}
                                    else: doc_input = Image.open(arq)
        ...
        if not st.session_state.wizard_ativo:
            if st.button("➕ Criar Novo Quadro Manualmente", type="primary"):
                st.session_state.wizard_ativo = True; st.rerun()
        ...
        if st.session_state.wizard_ativo:
            with st.container(border=True):
                ...
                if is_mercato_arch:
                    sup_opt = "Não"; soft_sel = "Sem Supervisório"
                    ...
                    if sup_opt == "Sim":
                        ...
                        if "COM certificação" in soft_sel: tipo_cfr_wizard = st.radio("Modalidade CFR-21 Part 11:", ["CFR21 Part 11 - Qualificável", "CFR21 Part 11 - Qualificado"], help=help_cfr_txt)
                ...
                if c_conf.button("🚀 Confirmar e Montar Quadro", use_container_width=True):
                    if not tag_q: st.warning("Insira uma TAG.")
                        ...
                        st.session_state.wizard_ativo = False; st.rerun()
                if c_canc.button("❌ Cancelar", use_container_width=True): st.session_state.wizard_ativo = False; st.rerun()

        st.write("")
        if st.session_state.confirmar_limpar:
            st.warning("⚠️ Deseja sair e PERDER todo o preenchimento não salvo nesta tela?")
            c_sim, c_nao = st.columns(2)
            if c_sim.button("✔️ Sim, Apagar Tudo"): st.session_state.paineis_auto = []; st.session_state.nome_projeto_orcamento = ""; st.session_state.confirmar_limpar = False; st.rerun()
            if c_nao.button("❌ Cancelar"): st.session_state.confirmar_limpar = False; st.rerun()
                ...
                if p_data['grupos_equipamentos']:
                    ...
                    for g_idx, g_data in enumerate(p_data['grupos_equipamentos']):
                        with abas_equipamentos[g_idx]:
                            ...
                            is_monitoramento = st.checkbox("📍 Exclusivo para Monitoramento/Controle Passivo (Sem chaves Auto/Manual)", value=st.session_state.get(f"chk_mon_{p_data['id']}_{g_idx}", auto_mon_default), key=f"chk_mon_{p_data['id']}_{g_idx}")
    ...
    with aba_resumo:
        ...
        for p in st.session_state.paineis_auto:
            ...
            # --- CORREÇÃO DO NAMEERROR (VARIÁVEIS BLINDADAS ANTES DA STRING) ---
            str_eqs_nome = ", ".join(l_eq_nm) if l_eq_nm else "Equipamentos"
            str_ctrls_desc = ", ".join(ctrl_desc) if ctrl_desc else "Controlador Padrão"
            ...
            txt_p = f"Sistema para {str_eqs_nome}{t_extra}.\nQuadro [TAG: {p['nome']}] {i_desc}, em {arq_at.replace(' - Linha mais econômica', '')} ({str_ctrls_desc}), {sup_desc}. Visualização e controle de:\n• Status de operação.\n"
        ...
        if total_geral > 0:
            ...
            expl = []
            if not df_i.empty:
                expl.append(pd.DataFrame([{"Categoria": "", "Item": "INSTRUMENTAÇÃO", "Preço Unit.": "-", "Qtd": "-", "Custo Total": "-"}]))
                expl.append(df_i.groupby(['Categoria', 'Item'], as_index=False).agg({'Preço Unit.': 'first', 'Qtd': 'sum', 'Custo Total': 'sum'}))
                expl.append(pd.DataFrame([{"Categoria": "SUBTOTAL", "Item": "INSTRUMENTAÇÃO", "Preço Unit.": "-", "Qtd": "-", "Custo Total": s_i}]))
            
            if not df_h.empty:
                expl.append(pd.DataFrame([{"Categoria": "", "Item": "HARDWARE", "Preço Unit.": "-", "Qtd": "-", "Custo Total": "-"}]))
                expl.append(df_h.groupby(['Categoria', 'Item'], as_index=False).agg({'Preço Unit.': 'first', 'Qtd': 'sum', 'Custo Total': 'sum'}))
                expl.append(pd.DataFrame([{"Categoria": "SUBTOTAL", "Item": "HARDWARE", "Preço Unit.": "-", "Qtd": "-", "Custo Total": s_h}]))
            
            if not df_s.empty:
                expl.append(pd.DataFrame([{"Categoria": "", "Item": "SOFTWARE", "Preço Unit.": "-", "Qtd": "-", "Custo Total": "-"}]))
                expl.append(df_s.groupby(['Categoria', 'Item'], as_index=False).agg({'Preço Unit.': 'first', 'Qtd': 'sum', 'Custo Total': 'sum'}))
                expl.append(pd.DataFrame([{"Categoria": "SUBTOTAL", "Item": "SOFTWARE", "Preço Unit.": "-", "Qtd": "-", "Custo Total": s_s}]))
            
            if not df_sr.empty:
                expl.append(pd.DataFrame([{"Categoria": "", "Item": "SERVIÇOS", "Preço Unit.": "-", "Qtd": "-", "Custo Total": "-"}]))
                expl.append(df_sr)
                expl.append(pd.DataFrame([{"Categoria": "SUBTOTAL", "Item": "SERVIÇOS", "Preço Unit.": "-", "Qtd": "-", "Custo Total": s_sr}]))
