    ...
    with aba_auto:
        ...
        for p_idx, p_data in enumerate(st.session_state.paineis_auto):
            ...
            with st.expander(f"🎛️ Quadro: {p_data['nome']} - {p_data.get('arquitetura', '')}", expanded=(p_idx == 0)):
                ...
                # --- NOVIDADE DE NAVEGAÇÃO: ABAS POR EQUIPAMENTO (Várias Folhas) ---
                if p_data['grupos_equipamentos']:
                    ...
                    for g_idx, g_data in enumerate(p_data['grupos_equipamentos']):
                        with abas_equipamentos[g_idx]:
                            ...
                            # FLUXOGRAMA DINÂMICO VISUAL (Graphviz) INTELIGENTE BASEADO NA PLANILHA
                            with st.expander("👁️ Visualizar Diagrama P&ID (Lógica e TAGs)", expanded=True):
                                try:
                                    ...
                                    # INCLUSÃO GRÁFICA DA IHM
                                    if p_data.get('ihm') and "Cego" not in p_data['ihm']:
                                        ihm_nome = p_data["ihm"].replace('Mercato - ', '')
                                    ...
                                    for inst_f, q_f in g_data['instrumentos'].items():
                                        if q_f > 0:
                                            ...
                                            for idx_q in range(int(q_f)):
                                                ...
                                                # VARREDURA DE TAGS PARA APLICAR NO DIAGRAMA
                                                ...
                                                # FILTRO DE CAMPO VAZIO NA PLANILHA
                                                if has_in_pin and lbl_in and str(lbl_in).strip() != "" and str(lbl_in).strip() != "nan":
                                                    dot += f'  "{node_name}_in" [label="{prefix}{lbl_in}{lbl_suf}{str_tag_ctx}\\nTAG: {tag_inst}", color="#2B7BC4"];\n'
                                                    dot += f'  "{node_name}_in" -> "Controlador" [color="#2B7BC4"];\n'
                                                    has_inputs = True
                                                    
                                                if has_out_pin and lbl_out and str(lbl_out).strip() != "" and str(lbl_out).strip() != "nan":
                                                    dot += f'  "{node_name}_out" [label="{prefix}{lbl_out}{lbl_suf}{str_tag_ctx}\\nTAG: {tag_inst}", color="#E14D2A"];\n'
                                    ...
                                    if lbl_in_c and str(lbl_in_c).strip() != "" and str(lbl_in_c).strip() != "nan":
                                        dot += f'  "chave_in" [label="{lbl_in_c}\\nTAG: CH", color="#2B7BC4"];\n'
                                        dot += f'  "chave_in" -> "Controlador" [color="#2B7BC4"];\n'
                                        has_inputs = True
                                    if lbl_out_c and str(lbl_out_c).strip() != "" and str(lbl_out_c).strip() != "nan":
                                    ...
                                    # RENDERIZAÇÃO 100% VISUAL (OCULTA O CÓDIGO FONTE DO USUÁRIO)
                                    st.graphviz_chart(dot)
                                    
                                    # BOTÃO DE DOWNLOAD DA ARQUITETURA
                                    st.download_button(label="📥 Baixar Arquivo do Diagrama (.dot)", data=dot, file_name=f"Diagrama_{g_data['nome_grupo']}.dot", key=f"dl_dot_{p_data['id']}_{g_idx}")
                                except Exception as e:
