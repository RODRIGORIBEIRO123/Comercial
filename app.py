...
# Configuração da Inteligência Artificial (Google Gemini) - Corrigido para 1.5-pro
try:
...
# ==============================================================================
                ...
                if p_data['grupos_equipamentos']:
                    ...
                    for g_idx, g_data in enumerate(p_data['grupos_equipamentos']):
                        with abas_equipamentos[g_idx]:
                            ...
                            # Identificador Sênior de Motores para a Chave
                            tem_motor = any(mot in g_data['nome_grupo'].upper() for mot in ["UTA", "EXAUST", "VENT", "FANCOIL", "SPLIT", "SPLITÃO"])
                            ...
                            is_monitoramento = st.checkbox("📍 Este equipamento é exclusivo para monitoramento de salas/controle passivo (Sem chaves Auto/Manual)", value=st.session_state.get(f"chk_mon_{p_data['id']}_{g_idx}", auto_mon_default), key=f"chk_mon_{p_data['id']}_{g_idx}")
    ...
    with aba_resumo:
        ...
        for p in st.session_state.paineis_auto:
            ...
            # Garantindo a existência das variáveis de string
            ...
            txt_p = (
                ...
                f"baseado na tecnologia {arq_at.replace(' - Linha mais econômica', '')} ({str_ctrls_desc}), operando no modo {sup_desc}, "
            ...
            txt_com = f"**Sistema completo de automação [TAG: {p['nome']}]**, construído com arquitetura de controladores **{arq_at.replace(' - Linha mais econômica', '')}** ({str_ctrls_desc}). O sistema operará de forma **{sup_desc}**, {ihm_desc}.\n\n"
        ...
        subtotal_inst = df_i['Custo Total'].sum() if not df_i.empty else 0
        subtotal_hw = df_h['Custo Total'].sum() if not df_h.empty else 0
        subtotal_sw = df_s['Custo Total'].sum() if not df_s.empty else 0
