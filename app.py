...
try:
    if "GEMINI_API_KEY" in st.secrets:
        ...
        # Modelo retornado para o 1.5-pro para evitar erro 404 (not found in v1beta)
...
# ==============================================================================
    ...
    with aba_auto:
        with st.expander("🔮 Módulo Inteligente: Extração com IA Sênior (Gemini Vision)", expanded=True):
            ...
            arquivos_diagrama = st.file_uploader("Carregar Diagrama Técnico / P&ID (Permite Múltiplos):", type=["pdf", "png", "jpg", "jpeg"], accept_multiple_files=True)
            if arquivos_diagrama:
                st.info(f"💡 {len(arquivos_diagrama)} arquivo(s) carregado(s). Verifique as configurações antes da análise.")
                ...
                if is_merc_ia:
                    ...
                    if sup_opt_ia == "Sim":
                        ...
                        if "COM certificação" in soft_sel_ia:
                            tipo_cfr_ia = c_ia2.radio("Modalidade CFR-21 Part 11:", ["CFR21 Part 11 - Qualificável", "CFR21 Part 11 - Qualificado"], help=help_cfr_txt)
                ...
                if st.button("🪄 Analisar Documentos com IA", type="primary"):
                    if not ia_disponivel:
                        ...
                        with st.spinner("A IA está analisando os diagramas P&ID..."):
                            for arq in arquivos_diagrama:
                                try:
                                    ...
                                    prompt = """Atue como Engenheiro de Automação Sênior. Analise este diagrama P&ID de HVAC detalhadamente.
                                    Faça uma contagem exata e responda APENAS com um JSON estrito, seguindo esta estrutura:
                                    {
                                        "utas": 0, // Contar máquinas principais
                                        "tipo_resfriamento": "Expansão Direta", // Deve ser "Expansão Direta" ou "Água Gelada". Analise a presença de válvulas de água ou condensadoras.
                                        "exaustores": 0, // Contar exaustores/ventiladores isolados (TAGs VE, EX, etc)
                                        "salas": 0, // Contar salas monitoradas
                        ...
                        st.success("Análise Sênior concluída! Valide os dados abaixo antes de gerar os quadros.")

                st.markdown("##### 🔍 Validação da Extração (Ajuste Manual)")
                mapa_arquivos = []
                for i, arq in enumerate(arquivos_diagrama):
                    with st.container(border=True):
                        ...
                        tipo_resf_padrao = 0 if dados_arq.get("tipo_resfriamento", "Expansão Direta") == "Expansão Direta" else 1
                        tipo_resfriamento = c1_b.selectbox("Resfriamento da UTA:", ["Expansão Direta", "Água Gelada"], index=tipo_resf_padrao, key=f"ia_tr_{i}")
                        ...
                        ops_filtros = ["G4 (PSH)", "M5 (PSH)", "M5 (PDT)", "F9 (PDT)", "H13/H14 (PDT)"]
                        ...
                        tag_dest = c6.text_input("TAG do Quadro Destino:", value=sugestao_quadro, key=f"ia_t_{i}")
                        
                        mapa_arquivos.append({
                            "arq_name": arq.name, "tag_quadro": tag_dest, "utas": qtd_utas, 
                            "tipo_resfriamento": tipo_resfriamento, "exaustores": qtd_exaustores, 
                            "salas": qtd_salas, "filtros": tem_filtros, "resistencia": tem_resistencia
                        })
                ...
                if st.button("🪄 Gerar Quadros com Extração Validada"):
                    ...
                    for tag_quadro, listas_configs in quadros_agrupados.items():
                        ...
                        for config in listas_configs:
                            ...
                            if config["utas"] > 0:
                                i_u = {k: 0 for k in st.session_state.de_para_diagrama.keys()}
                                ...
                                if config["tipo_resfriamento"] == "Água Gelada":
                                    i_u["Válvula de controle de água gelada proporcional (TCV)"] = 1
                                else:
                                    i_u["Relé de Corrente - Status Compressor (TC)"] = 2
                                ...
                                if "M5 (PDT)" in config["filtros"]: i_u["Transmissor de pressão diferencial (monitorar os filtros M5) (PDT)"] = 1
                            ...
                            if config["exaustores"] > 0:
                                i_e = {k: 0 for k in st.session_state.de_para_diagrama.keys()}
                            ...
                            if config["salas"] > 0:
                                i_s = {k: 0 for k in st.session_state.de_para_diagrama.keys()}
        ...
        if st.session_state.wizard_ativo:
            with st.container(border=True):
                ...
                ihm_selecionada = st.radio("2. IHM local?", opcoes_ihm_wizard, horizontal=True)
                ...
                if is_mercato_arch:
                    ...
                    if sup_opt == "Sim":
                        ...
                        soft_sel = st.selectbox("Software de Supervisão:", opcoes_soft)
                        if "COM certificação" in soft_sel:
                            tipo_cfr_wizard = st.radio("Modalidade CFR-21 Part 11:", ["CFR21 Part 11 - Qualificável", "CFR21 Part 11 - Qualificado"], help=help_cfr_txt)
                ...
                if c_conf.button("🚀 Confirmar e Montar Quadro", use_container_width=True):
                    if not tag_q: st.warning("Insira uma TAG.")
                        ...
                        novos_inst = {k: 0 for k in st.session_state.de_para_diagrama.keys()}
                        grupos_equip = []
                        if config_opt == "Usar Padrão Existente (Kits)":
                        ...
                        st.session_state.paineis_auto.insert(0, {"id": str(uuid.uuid4()), "nome": tag_q, "tipo": tipo_q, "supervisorio": soft_sel, "arquitetura": arquitetura_opt, "tipo_cfr": tipo_cfr_wizard, "modo_config": config_opt, "ihm": ihm_selecionada, "sobra_20": sobra_opt, "calibracao": calibracao_opt, "grupos_equipamentos": grupos_equip})
                        st.session_state.wizard_ativo = False
                        st.rerun()
        ...
        for p_idx, p_data in enumerate(st.session_state.paineis_auto):
            ...
            with st.expander(f"🎛️ Quadro: {p_data['nome']} - {p_data.get('arquitetura', '')}", expanded=(p_idx == 0)):
                ...
                c_ihm_painel.markdown(f"<div style='padding-top:10px; color:#555;'><b>IHM:</b> {p_data.get('ihm', 'Cego')}</div>", unsafe_allow_html=True)
                ...
                if p_data['grupos_equipamentos']:
                    ...
                    for g_idx, g_data in enumerate(p_data['grupos_equipamentos']):
                        with abas_equipamentos[g_idx]:
                            ...
                            tem_motor = any(mot in g_data['nome_grupo'].upper() for mot in ["UTA", "EXAUST", "VENT", "FANCOIL", "SPLIT", "SPLITÃO", "BOMBA"])
                            ...
                            with st.container():
                                for inst, q in g_data['instrumentos'].items():
                                    ...
                                    if is_mercato_quadro:
                                        ...
                                        if not modelo_mcp: 
                                            st.error(f"⚠️ CAPACIDADE EXCEDIDA para a linha Mercato.")
                            ...
                            with st.expander("👁️ Visualizar Diagrama P&ID (Lógica e TAGs)", expanded=False):
                                ...
                                node_idx = 0
                                group_boxes = is_monitoramento or ("EXAUST" in g_data["nome_grupo"].upper())
                                    ...
                                    if q_f > 0:
                                        ...
                                        if group_boxes:
                                            ...
                                            if has_in_pin and lbl_in_limpo.strip() not in ["", "nan"]:
                                                ...
                                                has_inputs = True
                                            if not is_monitoramento and has_out_pin and lbl_out_limpo.strip() not in ["", "nan"]:
                ...
                for g_data in p_data['grupos_equipamentos']:
                    ...
                    for inst, q in g_data['instrumentos'].items():
                        io_v = REGRA_IO.get(inst, {"AI": 0, "AO": 0, "DI": 0, "DO": 0})
                        raw_ai_painel += q * io_v["AI"] * qtd_atual_calc
        ...
        if st.session_state.paineis_auto:
            ...
            if c_bot_sair.button("🗑️ Sair (Limpar)", type="secondary", use_container_width=True): 
    ...
    with aba_resumo:
        ...
        for p in st.session_state.paineis_auto:
            ...
            is_siem = 'Siemens' in arquitetura_atual
            is_merc = 'Mercato' in arquitetura_atual
            is_sch = 'Schneider' in arquitetura_atual
            
            tem_s20 = p.get('sobra_20', 'Não') == 'Sim'
            cal_ativa = p.get('calibracao', 'Não') == 'Sim'
            ...
            for g_idx, g in enumerate(p['grupos_equipamentos']):
                ...
                tem_motor = any(mot in g['nome_grupo'].upper() for mot in ["UTA", "EXAUST", "VENT", "FANCOIL", "SPLIT", "BOMBA"])
                auto_mon_default = False if tem_motor else True
                ...
                for inst, qtd in g['instrumentos'].items():
                    if qtd > 0:
                        ...
                        elif "on/off" in inst.lower() or "proporcional" in inst.lower(): f_i = "Controle de Válvulas"
                        ...
                        elif "co2" in inst.lower(): f_i = "Medição da Qualidade do Ar (CO2)"
                        elif "fluxo" in inst.lower() or "fs" in inst.lower() or "cf" in inst.lower(): f_i = "Confirmação de Fluxo de Água"
            ...
            rs_ai = math.ceil(r_ai*0.2) if tem_s20 else 0
            rs_ao = math.ceil(r_ao*0.2) if tem_s20 else 0
            rs_di = math.ceil(r_di*0.2) if tem_s20 else 0
            rs_do = math.ceil(r_do*0.2) if tem_s20 else 0
            ...
            if t_hw > 0:
                if is_merc:
                    ...
                    if is_siem:
                        c_siem += p_cx
                        hw_s = dimensionar_siemens_1200(r_ai+rs_ai, r_ao+rs_ao, r_di+rs_di, r_do+rs_do) if '1200' in arquitetura_atual else dimensionar_siemens_1500(r_ai+rs_ai, r_ao+rs_ao, r_di+rs_di, r_do+rs_do)
                        for i_hw, q_hw in hw_s.items():
                            if q_hw > 0:
                                ...
                                linhas_hw.append({"Categoria": "Hardware", "Item": f"{i_hw} ({p['nome']})", "Preço Unit.": pr, "Qtd": q_hw, "Custo Total": q_hw * pr})
                                c_siem += (q_hw * pr)
                    else:
                        c_sch += p_cx
                        c36, c24, c18, c15 = dimensionar_controladores(t_hw)
                        if c36>0: 
                            controladores_desc_lista.append(f"{c36}x MP-C-36A")
                            linhas_hw.append({"Categoria": "Hardware", "Item": f"MP-C-36A ({p['nome']})", "Preço Unit.": st.session_state.precos_banco.get("MP-C-36A", 9459.0), "Qtd": c36, "Custo Total": c36*st.session_state.precos_banco.get("MP-C-36A", 9459.0)})
                            c_sch += c36*st.session_state.precos_banco.get("MP-C-36A", 9459.0)
                        if c24>0: 
                            controladores_desc_lista.append(f"{c24}x MP-C-24A")
                            linhas_hw.append({"Categoria": "Hardware", "Item": f"MP-C-24A ({p['nome']})", "Preço Unit.": st.session_state.precos_banco.get("MP-C-24A", 7290.0), "Qtd": c24, "Custo Total": c24*st.session_state.precos_banco.get("MP-C-24A", 7290.0)})
                            c_sch += c24*st.session_state.precos_banco.get("MP-C-24A", 7290.0)
                        if c18>0: 
                            controladores_desc_lista.append(f"{c18}x MP-C-18A")
                            linhas_hw.append({"Categoria": "Hardware", "Item": f"MP-C-18A ({p['nome']})", "Preço Unit.": st.session_state.precos_banco.get("MP-C-18A", 5185.0), "Qtd": c18, "Custo Total": c18*st.session_state.precos_banco.get("MP-C-18A", 5185.0)})
                            c_sch += c18*st.session_state.precos_banco.get("MP-C-18A", 5185.0)
                        if c15>0: 
                            controladores_desc_lista.append(f"{c15}x MP-C-15A")
                            linhas_hw.append({"Categoria": "Hardware", "Item": f"MP-C-15A ({p['nome']})", "Preço Unit.": st.session_state.precos_banco.get("MP-C-15A", 4649.0), "Qtd": c15, "Custo Total": c15*st.session_state.precos_banco.get("MP-C-15A", 4649.0)})
                            c_sch += c15*st.session_state.precos_banco.get("MP-C-15A", 4649.0)
                ...
                if s_tp != "Sem Supervisório":
                    if is_sch:
                        ...
                        linhas_hw.append({"Categoria": "Hardware", "Item": f"AS-P/AS-B ({p['nome']})", "Preço Unit.": pr_as, "Qtd": 1, "Custo Total": pr_as})
                        c_sch += pr_as
            ...
            # --- BLINDAGEM DO NAMEERROR ---
            str_eqs_nome = ", ".join(lista_equip_nomes) if lista_equip_nomes else "Equipamentos"
            ...
            txt_p = (
                f"Sistema de automação dedicado para controle de {str_eqs_nome}{t_extra}.\n\n"
            ...
            if t_cfr_p == 'CFR21 Part 11 - Qualificável':
            ...
            elif t_cfr_p == 'CFR21 Part 11 - Qualificado':
            ...
            for desc_inst, qt_inst, func_inst in l_inst_det:
        ...
        if t_geral > 0:
            ...
            df_disp = df_exp.copy()
            df_disp['Preço Unit.'] = df_disp['Preço Unit.'].apply(format_currency)
            df_disp['Custo Total'] = df_disp['Custo Total'].apply(format_currency)
            st.dataframe(df_disp, use_container_width=True)
            
            with st.expander("📄 Gerar Descritivo Detalhado para Proposta Comercial"):
                st.markdown("<div style='background-color:#E3F2FD; padding:20px; border-radius:10px;'>", unsafe_allow_html=True)
                for t_c in desc_comercial: 
                    st.markdown(t_c)
            ...
            if not df_p.empty: 
                df_p = pd.concat([df_p, pd.DataFrame([{"Painel": "TOTAL", "Grupo/Equipamento": "-", "Instrumento": "-", "Quantidade Total": pd.to_numeric(df_p['Quantidade Total'], errors='coerce').fillna(0).sum(), "DI": df_p['DI'].sum(), "DO": df_p['DO'].sum(), "AI": df_p['AI'].sum(), "AO": df_p['AO'].sum()}])], ignore_index=True)
            ...
            ws1.cell(1, 3, "DESCRIÇÃO DE SISTEMAS").font = Font(name="Arial", size=12, bold=True, color="1C8590")
            ws1.cell(1,3).alignment = Alignment(horizontal="center", vertical="center")
            ws1.merge_cells("C2:E2")
            ws1.cell(2, 3, f"PROJETO: {np_hdr.upper()}").font = Font(name="Arial", size=10, bold=True)
            ws1.merge_cells("C3:E3")
            ws1.cell(3, 3, f"DATA: {datetime.now(fuso_br).strftime('%d/%m/%Y %H:%M')}").font = Font(name="Arial", size=10)
            ws1.merge_cells("C4:E4")
            ws1.cell(4, 3, f"RESPONSÁVEL: {st.session_state.nome_exibicao.upper()}").font = Font(name="Arial", size=10)
            for r in range(2, 5): 
                ws1.cell(r, 3).alignment = Alignment(horizontal="center", vertical="center")
            
            if ARQUIVO_LOGO:
                try:
                    from openpyxl.drawing.image import Image as OpxImg
                    img = OpxImg(ARQUIVO_LOGO)
                    img.width = 180
                    img.height = 50
                    ws1.add_image(img, "A1")
                except: pass
                
            fl_h = PatternFill(start_color="1C8590", end_color="1C8590", fill_type="solid")
            ft_h = Font(name="Arial", size=11, bold=True, color="FFFFFF")
            ...
            for ri, row in enumerate(dataframe_to_rows(df_exp, index=False, header=True), sr):
                ...
                if ri == sr:
                    for c_idx in range(1, 6): 
                        ws1.cell(ri, c_idx).fill = fl_h
                        ws1.cell(ri, c_idx).font = ft_h
                        ws1.cell(ri, c_idx).alignment = Alignment(horizontal="center", vertical="center")
                else:
                    is_sub = "SUBTOTAL" in str(ws1.cell(ri, 1).value) or "TOTAL" in str(ws1.cell(ri, 1).value)
                    is_tit = str(ws1.cell(ri, 1).value) == ""
                    for c_idx in range(1, 6):
                        cc = ws1.cell(ri, c_idx)
                        cc.font = Font(name="Arial", size=10, bold=(is_sub or is_tit))
                        if is_tit:
            ...
            cd = ws1.cell(er, 1, desc_final)
            cd.font = Font(name="Arial", size=10)
            cd.alignment = Alignment(vertical="top", wrap_text=True)
            cd.fill = PatternFill(start_color="F2F4F4", end_color="F2F4F4", fill_type="solid")
            for r in range(er, er+tcx+1):
                for c in range(1, 6): ws1.cell(r, c).border = bd_t
            for col in ws1.columns: 
                ws1.column_dimensions[get_column_letter(col[0].column)].width = max(max(len(str(c.value or '')) for c in col if c.row <= sr + len(df_exp)) + 4, 12)
                ...
                for ri, row in enumerate(dataframe_to_rows(df_p, index=False, header=True), 1):
                    for ci, val in enumerate(row, 1):
                        ...
                        if ri == 1: 
                            ...
                            c.alignment = Alignment(horizontal="center") if ci >= 4 else Alignment(horizontal="left")
                for col in ws2.columns: 
                    ws2.column_dimensions[get_column_letter(col[0].column)].width = max(max(len(str(c.value or '')) for c in col) + 4, 12)
            ...
            ir = 5
            todos_it = [r['Item'] for _, r in df_h.iterrows()] + [r['Item'] for _, r in df_i.iterrows()]
            marcas = {"SIEMENS": [], "SCHNEIDER": [], "MERCATO": [], "OUTROS": []}
            for it in set(todos_it):
                cnt = todos_it.count(it)
                iu = it.upper()
            ...
            for mn, il in marcas.items():
                if il:
                    for ni, qi, ui in il:
                        ws3.cell(ir, 1, mn).alignment = Alignment(horizontal="center")
                        ws3.cell(ir, 2, ni)
                        ws3.cell(ir, 3, qi).alignment = Alignment(horizontal="center")
                        ws3.cell(ir, 4, ui).alignment = Alignment(horizontal="center")
                        for cc in range(1, 5): 
                            ws3.cell(ir, cc).border = bd_t
                            ws3.cell(ir, cc).font = Font(name="Arial", size=10)
                        ir += 1
                        
            for col in ws3.columns: 
                ws3.column_dimensions[get_column_letter(col[0].column)].width = max(max(len(str(c.value or '')) for c in col) + 4, 12)
            ...
            if st.button("☁️ Salvar e Gerar Revisão", type="primary", use_container_width=True):
                if not st.session_state.nome_projeto_orcamento: 
                    st.warning("Preencha o Nome do Projeto no topo da página.")
                else:
                    try:
                        sh = conectar_google_sheets()
                        try: 
                        ...
                        st.cache_data.clear()
                        st.success(f"✅ Salvo com sucesso! Revisão gerada: {rev}")
                    except Exception as e: 
